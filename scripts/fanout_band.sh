...
for ck in "$RUN_DIR"/checkpoint-*; do
    ...
    sbatch --partition="$PARTITION" ${NODE:+--nodelist="$NODE"} --gres="$GRES" \
        ...
        --export=ALL,SIZE="$SIZE",KIND="$KIND"${BASE_MODEL:+,BASE_MODEL="$BASE_MODEL"},CKPTS="$ck",OUT="$o",ARM="${SIZE}_${KIND}",SAVE_LENS="$SAVE_LENS",REQUEUE_ON_GPU_FAIL=0${DIM_BATCH:+,DIM_BATCH=$DIM_BATCH}${CKA_TOKENS:+,CKA_TOKENS=$CKA_TOKENS} \
...
echo "                      --expect-run-dir $RUN_DIR \\"
echo "                      --out $REPO/eval/band/${SIZE}_${KIND}/band.jsonl"
